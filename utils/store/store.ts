...
export const useNetworkBalancesStore: () => NetworkBalancesState =
  create<NetworkBalancesState>((set, get) => ({
    balances: {},
    ...
    getTotalBalance: () =>
      Object.values(get().balances).reduce(
        (acc: number, val: { balance: number; networkId: number }) =>
          acc + val.balance,
        0,
      ),
  }));
...
export const useAddressStore = create<AddressState>((set) => ({
  ...
  setResolvedAddress: (address: Address) => set({ resolvedAddress: address }),
}));
